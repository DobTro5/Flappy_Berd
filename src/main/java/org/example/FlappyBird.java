...
public class FlappyBird extends JPanel implements ActionListener {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (gameOver) { // Если игра окончена
            ...
            g.drawString("Game Over", WIDTH / 4, HEIGHT / 2); // Сообщение об окончании игры
            g.drawString("Press SPACE to Restart", WIDTH / 8, HEIGHT / 2 + 40); // Подсказка для перезапуска
        }
    }
    ...
    private void spawnPipe() {
        int height = new Random().nextInt(300) + 100; // Генерация случайной высоты трубы
